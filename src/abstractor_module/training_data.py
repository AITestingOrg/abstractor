...
def first_name_examples():
    ...
    return np.array([(row['input'].capitalize(), {'entities': [(len(row['input']) - len(row['word']), len(row['input']), row['label'])]})
...
def email_examples():
    ...
    return np.array([(row['input'], {'entities': [(0, len(row['input']), row['label'])]})
